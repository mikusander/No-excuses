...
The dual-threshold hysteresis ($\tau_{\text{enter}} = 360$, $\tau_{\text{exit}} = 75$) eliminates chattering around a single threshold: the energy must rise \textit{well above} resting levels to enter the peak state, and must fall \textit{well below} the entry point to exit it. This wide separation is viable because the EMA-smoothed energy decays exponentially after each peak: from a typical peak of $E \approx 700$, the EMA passes through $\tau_{\text{exit}} = 75$ in approximately 5--6 samples ($\sim\! 100$~ms at 60~Hz), well within the inter-repetition pause. The intra-peak valley energies observed during calibration ($E_{\text{valley}} \approx 105$--$120$) reflect transient minima \textit{during} a rep, not the true resting baseline between reps, which drops to $E \approx 20$--$50$ when the device is stationary in the pocket. The complete state machine is illustrated in \cref{fig:hysteresis}.
...
\begin{itemize}
  ...
  \item Intra-peak valley energy (transient minima during a rep): $E_{\text{valley}} \approx 105$--$120$.
  \item True resting baseline between repetitions (device stationary in pocket): $E \approx 20$--$50$.
